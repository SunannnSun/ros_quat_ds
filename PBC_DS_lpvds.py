...
for i in range(int(duration / stepsize)):
    ...
    """
    Insert own code for fx 
    """

    end_pos = np.array(end_pos).reshape(-1, 1)
    # fx = [-2 * (end_pos[0] - 0.4), -2 * (end_pos[1] - 0.3), -2 * (end_pos[2] - 0.5)]

    fx = lpv_ds_pos.step(end_pos)[:, 0]
